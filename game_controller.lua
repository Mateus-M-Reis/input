...
-- Carrega o arquivo de mapeamento de forma robusta
local function loadGamepadMappings()
  local str = nil

  -- Tenta usar o sistema de arquivos do LÖVR se disponível
  if lovr and lovr.filesystem then
    if lovr.filesystem.read then
      str = lovr.filesystem.read("gamecontrollerdb.txt")
    elseif lovr.filesystem.getInfo then
      local file = lovr.filesystem.newFile("gamecontrollerdb.txt")
  ...
  -- Se ainda não encontrou, tenta usar o diretório do script
  if not str then
    ...
    if script_path:sub(1, 1) == "@" then
      ...
      if script_dir then
        local f = io.open(script_dir .. "gamecontrollerdb.txt", "r")
        if f then
          str = f:read("*all")
          f:close()
        end
...
glfw.glfwInit()
local mappings = loadGamepadMappings()
if mappings and mappings ~= "" then
  glfw.glfwUpdateGamepadMappings(mappings)
else
  print("Warning: gamecontrollerdb.txt not found. Gamepad mapping may not work correctly.")
end
